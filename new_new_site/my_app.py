...
def results():
    ...
    search_result = main_search(input_string, '/home/philinguistic/mysite/panorama_corpus.json')
...
def main_search(input_string, corpus_json):
    ...
    if len(search_tokens) == 0:
        ...
        # если в кавычках
            if (token.startswith("'") and input_string.endswith("'")) or (token.startswith('"') and input_string.endswith('"')):
                token = token.replace("'", "")
                token = token.replace('"', "")
                token = token.lower()
                search_tokens.append(token)
